...
class Gdatabase {
    ...
    def dbinit() {
        if (sql == null){sql = Sql.newInstance(dburl, dbusrnm, dbpasswd, dbdriver)
        println "Connection Successful..."}
        else {sql.close(); sql = Sql.newInstance(dburl, dbusrnm, dbpasswd, dbdriver)}
    }
    ...
    def crttbl(String tablename, List<String> columns, List<String> datatyp)
    {
        ...
        columns.eachWithIndex { val, int num ->
            ...
            //str<<"$val $datyp"
            if (datatyp[num] == null || datatyp[num] == "") {str << "$val varchar(255)"}
            else {datyp = datatyp[num]; str << "$val $datyp"}
        }
        String tmp = str.join(",")
        //check For Table
        ...
        else{println "Table Poochi Already Exists..."}//Table Exist...
    }
    ...
    {
         if(checktableexist(tablename)==false){println "The Table $tablename Was Not Found In The Database...";println "Data Cannot Be Inserted...";println "Suggestions: \n1.Try Creating The Table Using crttbl() Method...\n2.Try Manually Creating The Table..." ;return}

        def str = []
        def str1= []
        colnames.eachWithIndex { val1, idx ->
            str << val1
            if (values[idx]==null){str1<<""}else{str1<<values[idx]}
        }
         String col=str.join(",")
    ...
    def dbdroptable(String tablename){dbqry="drop table $tablename"
